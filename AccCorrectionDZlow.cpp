...
int main(int argc, char* argv[]) { 
  ...
  if(targetArr[0] == 'D') {
    ...
    fileDataName = Form(dataDirectory + "VecSum_%sDZlow.root", solidTarget);
  } else{
    m = n+1;
    fileDataName = Form(dataDirectory + "VecSum_%sDZlow.root", targetArr);
  }
  ...
  TFile* fileSimul  = new TFile(Form(dataDirectory "SimulTuple_%sDZlow.root", simulTarget), "READ");
  gROOT->cd();
  TFile* fileOutput = new TFile(dataDirectory + Form("Sys/DZlow/corr_data_Phi_%i%i.root", Q2Bin,
						      NuBin), "RECREATE");
  gROOT->cd();
