...
int main(int argc, char* argv[]) { 
  ...
  //TFile* fileSimul  = new TFile(Form("~/proyecto/Pt2Broadening_multi-pion/Data/SimulTuple_%sdeltaZ.root", simulTarget), "READ");
  gROOT->cd();
  TFile* fileOutput = new TFile(outputDirectory + Form("corr_data_Phi_%i%i.root", Q2Bin, NuBin), "RECREATE");
  gROOT->cd();
      ...
      for(int NuCounter = NuBin; NuCounter < NuBin + 1; NuCounter++) { // Loops in every Nu bin
        for(int ZhCounter = 0; ZhCounter < N_Zh; ZhCounter++) { // Loops in every Zh bin
          ...
          for(int Pt2Counter = 0; Pt2Counter < N_Pt2; Pt2Counter++) { // Loops in every Pt2 bin
            ...
            Pt2Cut_gen  = Form("Pt2_gen>%f&&Pt2_gen<%f", Pt2_BINS[Pt2Counter], Pt2_BINS[Pt2Counter+1]);
            Pt2Cut_rec  = Form("Pt2_rec>%f&&Pt2_rec<%f", Pt2_BINS[Pt2Counter], Pt2_BINS[Pt2Counter+1]);
